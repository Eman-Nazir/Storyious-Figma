    ...
    <div className="flex flex-col lg:flex-row lg:gap-16 mx-4 sm:mx-6 lg:mx-14 my-10 lg:my-14">
      ...
      <div className="flex-1 space-y-5">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold text-[var(--text-dark)]">
            Living with Migraine? These Simple Exercises from DWTS Judge Derek Hough May Help
        </h1>

        <div className="flex flex-wrap gap-2">
          <p className="text-sm text-[var(--pink-dark)]">5 Min Stories.</p>
          <p className="text-sm text-[var(--pink-dark)]">Bedtime Stories.</p>
          <p className="text-sm text-[var(--pink-dark)]">Moral Stories</p>
        </div>
<div className="flex flex-col sm:flex-row sm:justify-between border-t border-b border-[var(--text-gray-light)] py-3 gap-4">
        <div className="w-full flex flex-wrap justify-between items-center gap-y-2 text-[var(--text-gray-light)] text-sm sm:text-base">
     ...
     <div className="flex items-center gap-3">
         <Share />
          <LuCopyCheck className="text-[var(--text-gray-light)] w-6 h-6 rounded-sm border border-[var(--text-gray-light)] p-1" />
         </div>
     </div>
     </div>

        <p className="text-base sm:text-lg leading-relaxed text-justify text-[var(--text-gray)]">
          Agriculture has been the backbTwo of human sustenance since the dawn of <br className="hidden sm:block" /> civilization, providing the <span className="text-[var(--pink-dark)] text-lg">vital link</span> between the soil beneath his feet and the nourishment on his plate.
        </p>
        ...
        {[blog1, blog2, blog3].map((img, idx) => (
          <div key={idx} className="border border-[var(--gray-light)] rounded-md p-4 space-y-4">
            <div className="flex gap-3 items-center">
              <div className="bg-[color-mix(in srgb, var(--pink-dark) 10%, white)] pt-3 text-lg font-bold w-[48px] h-[50px] text-center rounded-md">{idx + 1}</div>
              <h1 className="text-lg font-bold">Tamás Hám-Szabó</h1>
            </div>
            <img src={img} alt={`blog${idx + 1}`} className="rounded-md my-4 w-full max-h-[425px] object-cover" />
            <h1 className="text-lg font-bold">Syed Abubaker Ali Gilani Designer?</h1>
            <p className="text-base sm:text-lg leading-relaxed text-justify text-[var(--text-gray)]">
              The content published on HowTests, including Two-Liner MCQs, blogs, and articles, is created for educational and informational purposes only to assist competitive exam aspirants in their preparation. While our experts and readers actively monitor and update the material to ensure accuracy, <span className="text-[var(--pink-dark)] text-lg">HowTests</span> does not guarantee absolute correctness, completeness, or timeliness of the content.
            </p>
            {idx < 2 && (
      ...
      <div className="hidden lg:flex flex-col space-y-5 w-[300px] mt-10">
        ...
        <div className="flex flex-col p-4 rounded-md space-y-3 border border-[var(--border-muted)]">
          <h1 className="text-[var(--text-gray-light)] font-semibold">Follow Us On</h1>
          <div className="flex flex-wrap justify-center gap-2">
            <a href="#" className="bg-white border border-[var(--border-muted)] text-[var(--text-gray-light)] p-2 rounded-md"><FaFacebookF /></a>
            <a href="#" className="bg-white border border-[var(--border-muted)] text-[var(--text-gray-light)] p-2 rounded-md"><FaXTwitter /></a>
            <a href="#" className="bg-white border border-[var(--border-muted)] text-[var(--text-gray-light)] p-2 rounded-md"><FaLinkedinIn /></a>
            <a href="#" className="bg-white border border-[var(--border-muted)] text-[var(--text-gray-light)] p-2 rounded-md"><FaPinterestP /></a>
            <a href="#" className="bg-white border border-[var(--border-muted)] text-[var(--text-gray-light)] p-2 rounded-md"><FaInstagram /></a>
          </div>
        </div>
        ...
        <div className="px-6 py-20 bg-[var(--bg-section)] text-center">
          ...
          <p className="text-sm text-[var(--text-gray)] mt-2">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Quas magni cupiditate aliquam quibusdam nostrum accusantium fuga delectus quo exercitatiTwom!
          </p>
        </div>
      </div>
...
export default BlogTwoDetailPage;
