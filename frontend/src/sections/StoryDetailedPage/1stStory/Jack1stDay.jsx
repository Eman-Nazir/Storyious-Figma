...
//         <li className="leading-relaxed">avoiding your triggers</li>
//         <li className="leading-relaxed">seeking out alternative therapies</li>
//         <li className="leading-relaxed">making certain lifestyle changes</li>
//       </ul>
//     </div>
...
const Jack1stDay = () => {
  ...
  axios
    ...
    .then((res) => {
      if (Array.isArray(res.data.data)) {
        setStory(res.data.data[0]); // pick the first story
      } else {
        setStory(res.data.data); // if it’s already a single object
      }
    })
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between border-t border-b border-[var(--gray-mid)] py-3 gap-4">
        <div className="w-full flex flex-wrap justify-between items-center gap-y-2 text-[var(--gray-mid)] text-sm sm:text-base">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
            ...
            <p className="flex items-center gap-1 whitespace-nowrap">
              <Eye className="w-4 h-4" /> {story.meta.views || "0"}
            </p>
            <p className="items-center gap-1 whitespace-nowrap hidden sm:flex">
              <GoComment className="w-5 h-5" /> {story.commentsCount} Comments
            </p>
          </div>
          <div className="flex items-center gap-3">
            <Share />
      ...
      {/* Featured Image from uploads */}
      {story.featuredImage && (
        <img
          src={`http://localhost:8000/uploads/${story.featuredImage}`}
          alt="Feature"
          className="rounded-md w-full object-cover my-3 shadow-sm max-h-[600px]"
        />
      )}
